...
const ZLD_TASK_LOG_ARCHIVED_TBL_NAME string = "zld_task_log_archived"
...
func CreateZldTaskLogArchivedTable() {
	s := "CREATE TABLE IF NOT EXISTS"
	s = fmt.Sprintf("%s `%s`", s, ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	...
	s = fmt.Sprintf("%s) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='归档工作包日志表';", s)
	//fmt.Println("s=", s)

	o := orm.NewOrm()
	_, err := o.Raw(s).Exec()
	if err == nil {
		fmt.Println("Create %s table SUCCESS!", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
		zllogs.WriteDebugLog("Create %s table ...... DONE", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	} else {
		fmt.Printf("Create err=%v\n", err)
		fmt.Println("Create table ERROR!")
		zllogs.WriteErrorLog("Create %s table ...... ERROR", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	}	
}

func DoInsertTaskLogArchivedTableItem(item *ZldTaskLogData) {
	// insert data to the table
	s := fmt.Sprintf("INSERT INTO `%s`", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	...
	if err == nil {
		...
		zllogs.WriteDebugLog("Insert a record to %s table ...... DONE", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	} else {
		fmt.Printf("err=%v\n", err)
		fmt.Println("mysql insert data have an ERROR!")
		zllogs.WriteErrorLog("Insert a record to %s table ...... ERROR", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
	}		
}

func AlreadyHaveTaskLogArchivedItem(id string) bool {
	s := fmt.Sprintf("SELECT * FROM `%s`", ZLD_TASK_LOG_ARCHIVED_TBL_NAME)
...
func InsertTaskLogArchivedTableItem(item *ZldTaskLogData) {
	// first, try to create the table
	CreateZldTaskLogArchivedTable()

	// No same sn item, do insert
	if !AlreadyHaveTaskLogArchivedItem(item.TaskId) {
		DoInsertTaskLogArchivedTableItem(item)
	}
}
