...
const ZLD_TASK_ARCH_TBL_NAME string = "zld_task_archived"
...
func CreateZldTaskArchivedTable() {
	s := "CREATE TABLE IF NOT EXISTS"
	s = fmt.Sprintf("%s `%s`", s, ZLD_TASK_ARCH_TBL_NAME)
	...
	s = fmt.Sprintf("%s) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='归档工作包表';", s)
	//fmt.Println("s=", s)

	o := orm.NewOrm()
	_, err := o.Raw(s).Exec()
	if err == nil {
		fmt.Println("Create %s table SUCCESS!", ZLD_TASK_ARCH_TBL_NAME)
		zllogs.WriteDebugLog("Create %s table ...... DONE", ZLD_TASK_ARCH_TBL_NAME)
	} else {
		fmt.Printf("Create err=%v\n", err)
		fmt.Println("Create table ERROR!")
		zllogs.WriteErrorLog("Create %s table ...... ERROR", ZLD_TASK_ARCH_TBL_NAME)
	}	
}

func DoInsertTaskArchivedTableItem(item *ZldTaskData) {
	// insert data to the table
	s := fmt.Sprintf("INSERT INTO `%s`", ZLD_TASK_ARCH_TBL_NAME)
	...
	s = fmt.Sprintf("%s ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%v', '%v', '%v', '%v', '%v', '%v', '%s', '%s');", s, item.TaskId, item.SponsorId, item.FarmId, item.CellId, item.PatchId, item.WorkerId, item.CheckerId, common.ZLD_TASK_STATE_ARCHIVED, item.Type, item.CreateTime, item.StartTime, item.EndTime, item.CheckTime, item.Score, item.UserComment, item.Comment)
	//fmt.Println("s=", s)
	...
	if err == nil {
		...
		zllogs.WriteDebugLog("Insert a record to %s table ...... DONE", ZLD_TASK_ARCH_TBL_NAME)
	} else {
		fmt.Printf("err=%v\n", err)
		fmt.Println("mysql insert data have an ERROR!")
		zllogs.WriteErrorLog("Insert a record to %s table ...... ERROR", ZLD_TASK_ARCH_TBL_NAME)
	}		
}

func AlreadyHaveTaskArchivedItem(id string) bool {
	s := fmt.Sprintf("SELECT * FROM `%s`", ZLD_TASK_ARCH_TBL_NAME)
...
func InsertTaskArchivedTableItem(item *ZldTaskData) {
	// first, try to create the table
	CreateZldTaskArchivedTable()

	// No same sn item, do insert
	if !AlreadyHaveTaskArchivedItem(item.TaskId) {
		DoInsertTaskArchivedTableItem(item)
	}
}

// func UpdateTaskItemInfo(item *ZldTaskData) error{
// 	s := fmt.Sprintf("UPDATE `%s`", ZLD_TASK_ARCH_TBL_NAME)
...
// 	_, err := o.Raw(s).Exec()
// 	if err == nil {
// 		zllogs.WriteDebugLog("Update record(TaskId=%s) in table %s  ...... DONE", item.TaskId, ZLD_TASK_ARCH_TBL_NAME)
// 	} else {
// 		zllogs.WriteErrorLog("Update record(TaskId=%s) in table %s  ...... ERROR", item.TaskId, ZLD_TASK_ARCH_TBL_NAME)
// 	}
